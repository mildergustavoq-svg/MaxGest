...
from Milder import mostrar_grafica_ventas_diarias, mostrar_grafica_ventas_hoy
...
class App(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkButton(self.menu, text="📊 Dashboard", font=("Segoe UI", 14), anchor="w", command=self.mostrar_dashboard).pack(pady=8, padx=15, fill="x")
        ctk.CTkButton(self.menu, text="📦 Almacén / Productos", font=("Segoe UI", 14), anchor="w", command=self.mostrar_productos).pack(pady=8, padx=15, fill="x")
        ctk.CTkButton(self.menu, text="🛒 Registrar Ventas", font=("Segoe UI", 14), anchor="w", command=self.mostrar_ventas).pack(pady=8, padx=15, fill="x")
        ctk.CTkButton(self.menu, text="📈 Reportes Predictivos", font=("Segoe UI", 14), anchor="w", command=self.mostrar_reportes).pack(pady=8, padx=15, fill="x")
        ctk.CTkButton(self.menu, text="📋 Historial de Informes", font=("Segoe UI", 14), anchor="w", fg_color="#1ABC9C", hover_color="#16A085", command=lambda: Minchola.inicializar_vista_informes(self)).pack(pady=8, padx=15, fill="x")
        ctk.CTkButton(self.menu, text="❌ Salir del Sistema", font=("Segoe UI", 14, "bold"), fg_color="#E74C3C", hover_color="#C0392B", command=self.destroy).pack(side="bottom", pady=20, padx=15, fill="x")
    ...
    def mostrar_productos(self):
        ...
        if productos:
            ...
            # === NUEVO BOTÓN DE EDITAR (AÑADE ESTA LÍNEA) ===
            ctk.CTkButton(self.contenido, text="✏️ Editar producto seleccionado", fg_color="#3498DB", hover_color="#2980B9", command=lambda: miguel.abrir_ventana_editar(self)).pack(pady=5)
        
        
    def agregar_producto(self):
        # Llamamos a tu validador externo pasando los textos de los campos entry
        es_valido, stock, p_c, p_v = miguel.validar_datos_alta(
            self.nombre_entry.get(),
            self.stock_entry.get(),
            self.precio_compra_entry.get(),
            self.precio_venta_entry.get()
        )
        
        # Si las validaciones de tu archivo externo pasan con éxito, se guarda en la BD
        if es_valido:
